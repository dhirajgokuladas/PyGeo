...
def test__intersect_given_sphere_ray__intersected_one_point_d_grater_than_0():
    assert (_intersect_ray_with_sphere(Ray(direction=Vector((2,0,0)),origin=Point((0,1,0))),Sphere(center=Point((0,0,0)),radius=1)).all()==np.array([[0.,1.,0.]]).all())
def test__intersect_given_sphere_ray__intersected_two_point_d_grater_then_0():
    assert (_intersect_ray_with_sphere(Ray(direction=Vector((1.2,-1.2,0)),origin=Point((0,1.2,0))),Sphere(center=Point((0,0,0)),radius=1)).all()==np.array([[1., 0., 0.],[0., 1., 0.]]).all())
